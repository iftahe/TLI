...
def format_clothing_recommendation(forecast: dict) -> str:
    ...
    if max_t >= 30:
        adults = "👕 טמפרטורות גבוהות מחר. מומלץ ללבוש בגדים קלים ומאווררים ולשתות הרבה מים"
    elif max_t >= 25:
        adults = "👕 מזג אוויר נעים מחר. מומלץ ללבוש חולצה קלה ומכנסיים קצרים"
    elif max_t >= 20 and min_t >= 15:
        adults = "🧥 נעים ביום, קריר בערב. מומלץ לקחת סוודר קל"
    elif max_t >= 15:
        adults = "🧥 טמפרטורות נמוכות יחסית. מומלץ ללבוש שכבות — חולצה ארוכה וז'קט"
    else:
        adults = "🧣 קר מחר. מומלץ ללבוש מעיל חם, צעיף וכובע"

    if rain >= 70:
        adults += "\n☔ סיכוי גבוה לגשם — מומלץ לקחת מטרייה"
    elif rain >= 40:
        adults += "\n🌂 יש סיכוי לגשם — מומלץ לקחת מטרייה"

    kids = ""
    if min_t < 15:
        kids = "🧒 לילדים: מומלץ שכבה נוספת וכובע"
    if rain >= 40:
        kids_rain = "מגפיים ומעיל גשם"
        if kids:
            kids += " + " + kids_rain
        else:
            kids = "🧒 לילדים: מומלץ " + kids_rain
